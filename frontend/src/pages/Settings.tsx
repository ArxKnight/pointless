...
function Admins({current}:{current:User}){
 ...
 async function save(u:UserAdmin){try{const e=edit[u.id]||{};await patch(`/users/${u.id}`,{username:e.username,display_name:e.display_name,email:e.email,is_admin:true,password:e.password});setEdit(prev=>{const n={...prev};delete n[u.id];return n});setMsg('Administrator updated.');await load()}catch(e:any){setMsg(e.message)}}
