...
def create_member(data:MemberCreate, db:Session=Depends(get_db), admin:User=Depends(require_admin)):
    ...
    if data.username and data.password:
        ...
        db.add(User(username=data.username,display_name=data.display_name,email=data.email,password_hash=hash_password(data.password),is_admin=data.is_admin,is_active=True))
